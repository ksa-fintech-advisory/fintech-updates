...
export default function SubscriptionForm({ locale }: { locale: string }) {
  ...
  const t = {
    title: isArabic ? 'احصل على الوصول الكامل' : 'Get Full Platform Access',
    subtitle: isArabic 
      ? 'استفد من جميع أدواتنا، والتقارير الحصرية، والخدمات الاستشارية لتنمية أعمالك.' 
      : 'Unlock all tools, exclusive market reports, and expert advisory services to grow your business.',
    plans: {
      pro: {
        name: isArabic ? 'برو' : 'Pro',
        price: isArabic ? '٤٩٩ ر.س / شهرياً' : 'SAR 499 / mo',
        features: isArabic 
          ? ['حاسبة الرسوم المتقدمة', 'فاحص الامتثال غير المحدود', 'تقارير السوق الشهرية', 'دعم عبر البريد الإلكتروني'] 
          : ['Advanced Fee Calculator', 'Unlimited Compliance Checks', 'Monthly Market Reports', 'Email Support']
      },
      enterprise: {
        name: isArabic ? 'مؤسسات' : 'Enterprise',
        price: isArabic ? 'تواصل معنا' : 'Contact Us',
        features: isArabic 
          ? ['كل مميزات برو', 'ساعات استشارية مخصصة', 'تكامل API مخصص', 'مدير حساب خاص'] 
          : ['Everything in Pro', 'Dedicated Advisory Hours', 'Custom API Integration', 'Dedicated Account Manager']
      }
    },
    form: {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl border border-primary-100 overflow-hidden flex flex-col md:flex-row my-12">
      {/* Plan Selection Side */}
      <div className="bg-slate-900 p-8 md:p-12 md:w-2/5 text-white flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-primary-100 mb-8 font-light">{t.subtitle}</p>
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className={`w-full text-start p-4 rounded-xl border transition-all ${
                plan === 'pro' 
                  ? 'bg-primary-600 border-primary-500 shadow-lg' 
                  : 'bg-white/5 border-white/10 hover:bg-white/10'
              }`}
            >
              <div className="flex justify-between items-center mb-1">
                <span className="font-bold">{t.plans.pro.name}</span>
                {plan === 'pro' && <div className="w-4 h-4 rounded-full bg-white flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-primary-600"></div></div>}
              </div>
              <div className="text-sm text-primary-200">{t.plans.pro.price}</div>
            </button>
            ...
            >
              <div className="flex justify-between items-center mb-1">
                ...
                {plan === 'enterprise' && <div className="w-4 h-4 rounded-full bg-white flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-primary-600"></div></div>}
              </div>
              <div className="text-sm text-primary-200">{t.plans.enterprise.price}</div>
            </button>
          </div>
        </div>

        <div className="relative z-10 mt-8 pt-8 border-t border-white/10">
          <h4 className="text-sm font-bold uppercase tracking-wider text-primary-300 mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input 
                required 
                type="text" 
                className="w-full px-4 py-3 rounded-xl border border-grey-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 transition-all outline-none"
          ...
          <button
            ...
            className={`w-full py-4 text-white font-bold rounded-xl shadow-lg transition-all transform hover:scale-[1.02] ${
              isLoading ? 'bg-primary-400 cursor-wait' : 'bg-gradient-to-r from-primary-600 to-primary-800 hover:from-primary-700 hover:to-primary-900'
            }`}
          >
