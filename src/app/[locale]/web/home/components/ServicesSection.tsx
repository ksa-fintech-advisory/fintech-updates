...
import { FiBriefcase, FiUsers, FiArrowRight, FiArrowLeft } from 'react-icons/fi';
...
export default function ServicesSection() {
  ...
  const contactHref = `/${locale}/web/contact`;
            ...
            return (
              <StaggerItem key={id}>
                <article className="flex h-full flex-col gap-4 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950 md:p-8">
                  ...
                  <Link
                    href={contactHref}
                    className="mt-auto inline-flex w-fit items-center gap-2 rounded-xl border border-zinc-300 px-5 py-3 text-sm font-bold text-zinc-900 transition-colors hover:border-primary-500/40 hover:bg-zinc-50 dark:border-zinc-600 dark:text-white dark:hover:border-primary-400/40 dark:hover:bg-zinc-900/80"
                  >
                    {t(`items.${id}.cta`)}
                    {ctaArrow}
                  </Link>
