...
import { calculateFees, validateRequest } from '@/services/fee-calculator/engine';
import type { FeeCalculationRequest, PricingData } from '@/services/fee-calculator/types';
  ...
  const handleSubmit = async (formData: any) => {
    ...
    try {
      const calcRequest: FeeCalculationRequest = {
        ...
        monthlyVolume: Number(formData.monthlyVolume) || 0,
        averageTransactionValue: Number(formData.averageTransactionValue) || 0,
        paymentMix: {
          mada: Number(formData.paymentMix?.mada) || 0,
          visa_mc_local: Number(formData.paymentMix?.visa_mc_local) || 0,
          visa_mc_international: Number(formData.paymentMix?.visa_mc_international) || 0,
        },
      };

      const validation = validateRequest(calcRequest, pricingData);
      if (!validation.valid) {
        const msg =
          validation.errors.join(', ') ||
          (isArabic ? 'بيانات غير صالحة' : 'Invalid request data');
        throw new Error(msg);
      }

      const lang = isArabic ? 'ar' : 'en';
      const data = calculateFees(calcRequest, pricingData, lang);
      setResults(data);
    } catch (err) {
