...
export default function RulesExplorerPage({ params: { locale } }: { params: { locale: string } }) {
  const isArabic = locale === 'ar';
      ...
      <section className="relative pt-32 pb-12 border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm z-10">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="mb-4 flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">
                <Link
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1 mb-6 rounded border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300">
                ...
                <span className="text-xs  font-bold uppercase tracking-widest">
                  {isArabic ? 'قاعدة_البيانات_التنظيمية' : 'REGULATORY_DATABASE'}
                </span>
              </div>
              <h1 className="text-3xl md:text-5xl font-black mb-4 tracking-tight leading-[1.16] md:leading-[1.14] text-zinc-900 dark:text-white">
                {isArabic ? 'مستكشف قواعد الامتثال' : 'Rulebook Explorer'}
              </h1>
              <p className="text-lg text-zinc-500 dark:text-zinc-400 font-light max-w-2xl leading-relaxed">
                {isArabic
                  ? 'ابحث وتصفح متطلبات هيئة السوق المالية حسب الوحدة ومستوى المخاطر.'
                  : 'Search and browse CMA requirements by module and risk level.'}
              </p>
            </div>
